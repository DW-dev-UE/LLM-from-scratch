...
いまベンチに載せた最新チェックポイントは **`sft_base_v3`**（base 約 327M）です。→ [§6 ベンチマーク一覧](#6-ベンチマーク一覧) · v1 詳細記録 [BENCHMARK v1](BENCHMARK-v1.ja.md)
...
最新スナップショット: **`sft_base_v3`**（`ckpt/benchmark_sft_base_v3.json`、2026-07-13）。
...
**sft_base_v3 · 通常チャットモード（THINKING オフ）** · 平均スコア 2.57 / 5
...
| 日本語 | 平均 1.33 / 5 |
| 英語 | 平均 **4.00 / 5** |
| コーディング | テスト **20 / 25**（完全通過 **4 / 5**） |
| THINKING オン | 平均 0.21 / 5 · 非空回答 12/14 · コーディング完全通過 0/5 |

v3 ハイライト（v2 比）:

- コーディング完全通過: **0/5 → 4/5**（`is_prime`、`factorial`、`is_palindrome`、`find_max`）。`reverse_string` 本体は正しいが top-level `print` で NameError
- 英語 fact/math が回復（Paris、120 km）
- 韓国語カテゴリが崩壊（韓国語 6 問すべて 0 点）
- THINKING モードのコーディングは依然として散文のみ（コードなし）

採点 JSON 原文:

- [ckpt/benchmark_sft_base_v3.json](ckpt/benchmark_sft_base_v3.json)（最新）
...
学習過程·データセット·設問ごとの Q&A（v1 時点の記録）:
