...
> [!IMPORTANT]
> **APEX-1（1B 級）— Pretrain + SFT 完了**
>
> 実測 **1,119.5M** パラメータ。モデル名 **APEX-1**(コード上の preset 名は `xl` — `model.py` のサイズ階層キーにすぎず、チェックポイントのファイル名は `sft_xl_v1` のように preset 基準で自動生成されます)。
>
> - **構造**: 24層 · d_model 2048 · GQA（16Q/4KV）+ RoPE（θ=500K）+ SwiGLU + RMSNorm · weight tying
> - **コンテキスト**: max 4096（学習は 2048）
> - **Pretrain**: bf16 · lr 3e-4 コサイン · 51K step（20B トークン）· コーパス v3-en 80.8GB（英語+コード）· vocab 32K・英語専用
> - **SFT**: `sft_xl_v1` · 8.4K step · lr 3e-5 · `pretrain_xl_v1` の上で SFT
> - **ベンチ（英語専用15問・greedy、[生データ](ckpt/benchmark_sft_xl_v1_raw.json)）**: no-thinking コーディング完全通過 **5/5**（テスト25/25）· QA 5/8 — thinking モードはむしろ低下してコーディング3/5（テスト15/25）· QA 4/8
> - **次の一手**: 冗長さ・繰り返しがボトルネック（over-length 8/10、repetition 0.032）、能力自体は十分と判断 → **RLVR へ進む決定**（[根拠](ckpt/AUTO_DECISION_xl_v1.txt)）
...
| [BENCHMARK v2](BENCHMARK-v2.ja.md) | APEX-1(1B)ベンチ・RLVR 決定根拠 |
...
2つの系列を並行しています。1B `xl` 系列(APEX-1)の最新は **`sft_xl_v1`**(pretrain+SFT 完了・RLVR 待ち)、327M `base` 系列の最新は **`sft_base_v6`** です。→ [§5 ベンチマーク一覧](#5-ベンチマーク一覧) · バージョン別詳細記録 [BENCHMARK v2](BENCHMARK-v2.ja.md)
...
2つの系列が並行しています: **1B `xl`**(APEX-1、英語専用)と **327M `base`**(韓/日/英 多言語)。セットも別物です。

### 5.1 1B `xl` 系列(APEX-1)⭐ 主力

英語専用15問 × THINKING on/off(コーディング5問は327Mセットと同一問題)。  
最新かつ唯一のスナップショット: **`sft_xl_v1`**(`ckpt/benchmark_sft_xl_v1_raw.json`、2026-07-22)。Pretrain 51K step(20B トークン)+ SFT 8.4K step 完了。
...
能力(コーディング5/5満点)は十分と判断し、次の段階は追加 SFT ではなく **RLVR** に決定しました。

詳細記録: [BENCHMARK-v2.ja.md](BENCHMARK-v2.ja.md)

<details>
<summary><b>5.2 327M `base` 系列(展開して見る)</b></summary>
